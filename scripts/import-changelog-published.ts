...
const changelogData = [
  // --- 2026-06-19 ---
  {
    ...
    changes: [
      { text: '管理画面の全コードエディタ（HTMLソース、HTMLホスティング、JSONメタデータ）を CodeMirror 6（@uiw/react-codemirror）に統一。ライト/ダークテーマの自動追従と VS Code 風ダークテーマを適用' }
    ]
  },
  {
    version: 'v1.5.0',
  ...
  // --- 2026-05-15 ---
  {
    ...
    changes: [
      ...
      { text: 'ダーク・ニューモーフィズム調 of Landing Page、About、Services ページ（ヒアリングシート）を初期実装' }
    ]
  }
]
...
async function run() {
  ...
  for (const doc of existing.docs) {
    ...
    console.log(`- Deleted changelog entry: [${doc.date}] (ID: ${doc.id})`)
  }

  console.log(`\nStarting import of ${changelogData.length} published entries...`)
  let count = 0
  for (const entry of changelogData) {
    try {
      ...
      console.log(`✓ Created Published: [${doc.date}] ${doc.category} (ID: ${doc.id})`)
      count++
    } catch (err) {
      console.error(`✗ Failed to import entry: ${entry.date} - ${entry.category}`, err)
    }
  }
  console.log(`\nImport completed successfully. Re-imported ${count}/${changelogData.length} published entries.`)
  process.exit(0)
}

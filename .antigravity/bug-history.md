...
- **Prevention Note**:
  - Payload v3 の `lexicalEditor({ features })` は **関数コールバック形式を使うこと**。`defaultEditorFeatures` の直接 sprea### [2026-05-30 01:40] Bug: Custom Monaco Code Block button does not appear or fails to insert in editor toolbar
- **Error**:
...
�加する際は、`admin.importMap.generators` に `BlocksFeatureClient` 明示登録ジェネレーターを合わせて追加すること。

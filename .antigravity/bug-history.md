...
### [2026-05-30 20:30] Bug: CodeMirror 6 custom selection color not applied — CSS injection / EditorView.theme() override both fail
- **Error**: Selection highlight in the CodeMirror editor remained the default vscodeDark blue (`#6199ff2f`) even after injecting a `<style>` tag with `!important` and after adding a separate `EditorView.theme()` extension.
- **Root Cause**: `@uiw/codemirror-themes`'s `createTheme` (used internally by `vscodeDark`) emits selection color as `background: '<value>' !important` inside a **scoped CSS class** (e.g., `.ͼ1 .cm-selectionBackground`). This `!important` scoped rule is injected into `<head>` **after** any manually injected `<style>` tags, so same-specificity `!important` rules lose to it due to source order. A separate `EditorView.theme()` extension placed in `extensions[]` also fails because the theme class order depends on how `@uiw/react-codemirror` internally merges the `theme` prop and `extensions`; there is no guarantee our extension wins.
- **File(s) Modified**: `src/components/MobileFullscreenEditor/index.tsx`
- **Fix Summary**: Abandoned the CSS override approach entirely. Used `vscodeDarkInit({ settings: { selection: '...' } })` to produce a **single combined theme** where the selection color is baked in from the start — using the same code path that generates the default blue. This avoids any cascade conflict.
- **Prevention Note**: For any `@uiw/codemirror-theme-*` theme that uses `createTheme` internally, **never** try to override `selection`/`selectionMatch` colors via external CSS or a stacked extension. Always use the theme's own `*Init()` function with `settings` overrides. If a theme doesn't expose an init function, fork it or use `createTheme` directly.

### [2026-05-30 20:00] Bug: Admin HTML/CSS editor slow & visually glitchy (Prism.js overlay approach)
- **Error**: The custom code editor in `MobileFullscreenEditor` was laggy during typing and had visible misalignment between the syntax highlight layer and the actual text cursor position. On long documents the overlay `<pre>` drifted out of sync with the textarea scroll position.
- **Root Cause**: The implementation used a "transparent textarea + absolute `<pre>` overlay" pattern with Prism.js. Three compounding issues: (1) `Prism.highlight()` reprocessed the **entire** text string on every keystroke — O(n) cost; (2) `dangerouslySetInnerHTML` forced full DOM reconciliation of the `<pre>` on every change; (3) the overlay's scroll position had to be kept in sync manually, which drifted on word-wrap and long lines.
- **File(s) Modified**: `src/components/MobileFullscreenEditor/index.tsx`
- **Fix Summary**: Replaced the entire overlay stack with **CodeMirror 6** via `@uiw/react-codemirror`. CodeMirror uses incremental parsing and virtual DOM rendering — only visible lines are processed, and internal state updates bypass React's reconciler entirely. The textarea + pre overlay was removed completely.
- **Prevention Note**: The "transparent textarea + highlighted `<pre>` overlay" pattern is fundamentally unsuitable for production editors beyond toy-level complexity. For any real code editing UI in this project, always use CodeMirror 6. See `.antigravity/knowledge/backend/admin-ui/syntax-highlighter-formatter.md` for the canonical setup pattern.
